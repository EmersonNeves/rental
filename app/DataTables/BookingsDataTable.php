...
class BookingsDataTable extends DataTable
{
    public function ajax()
    {
        ...
        return datatables()
            ...
            //CUSTOM
            ->addColumn('payment',function($bookings){
              return ucfirst(strtolower($bookings->payment_status ?? ''));
            })
            ->addColumn('action', function ($bookings) {
                return '<a href="' . url('admin/bookings/detail/' . $bookings->id) . '" class="btn btn-sm svbtn" title="Detail View"><i class="fa fa-share"></i></a>&nbsp;';
            })
            ->rawColumns(['host_name','guest_name','total_amount','property_name', 'action', 'status'])
            ->make(true);
    }
    ...
    public function query()
    {
        ...
        $bookings = Bookings::join('properties', function ($join) {
                        ...
                        ->select(['bookings.id as id', 'u.first_name as host_name', 'users.first_name as guest_name', 'bookings.property_id as property_id', 'properties.name as property_name', DB::raw('CONCAT(currency.symbol, bookings.total) AS total_amount'), 'bookings.status', 'bookings.created_at as created_at', 'bookings.updated_at as updated_at', 'bookings.start_date', 'bookings.end_date', 'bookings.guest', 'bookings.host_id', 'bookings.user_id', 'bookings.total', 'bookings.currency_code', 'bookings.service_charge', 'bookings.host_fee', 'bookings.iva_tax', 'bookings.accomodation_tax',
                                  //CUSTOM
                                  DB::raw("(SELECT status FROM transactions WHERE booking_id = bookings.id ORDER BY created_at DESC LIMIT 1) as payment_status")]);
        if (isset($user_id)) {
        ...
        if (!empty($payment) && $payment != 'all'){
          $bookings->whereIn('bookings.id',function($q) use($payment)
          {
            $q->select('transactions.booking_id')
              ->from('transactions')
              ->where('transactions.status',strtoupper($payment));
          });
        }
    ...
    public function html()
    {
        return $this->builder()
            ...
            //CUSTOM
            ->addColumn(['data' => 'payment', 'name' => 'bookings.payment', 'title' => 'Payment'])
