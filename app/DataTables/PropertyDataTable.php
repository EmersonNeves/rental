...
class PropertyDataTable extends DataTable
{
        ...
        return datatables()
            ...
            ->addColumn('action', function ($properties) {
                ...
                if (Common::has_permission(\Auth::guard('admin')->user()->id, 'edit_properties')) {
                    $edit = '<a href="' . url('admin/listing/' . $properties->properties_id) . '/basics" class="btn btn-xs svbtn"><i class="fa fa-pencil"></i></a>';
                }
                if (Common::has_permission(\Auth::guard('admin')->user()->id, 'delete_property')) {
                    ...
                    {
                        $delete = '<a href="' . url('admin/delete-property/' . $properties->properties_id) . '" class="btn btn-xs svbtn delete-warning"><i class="fa fa-trash"></i></a>';
                    }
                }
                return $edit . $delete;
            })
    ...
    public function query()
    {
        ...
        $query->where('properties.type', '=', 'property');
