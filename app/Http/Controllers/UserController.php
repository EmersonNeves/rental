...
class UserController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $bookings               = Bookings::select('payment_method_id', 'currency_code',
                                DB::raw('(total - service_charge - iva_tax - accomodation_tax) as total'), 'created_at')->where(['host_id' => $user_id, 'status' => 'Accepted']);
        ...
        $data['user_verification'] = UsersVerification::where('user_id', $user_id)->first();

        $data['reviews_from_guests'] = Reviews::with('users', 'properties')->where(['receiver_id'=>$user_id, 'reviewer'=>'guest'])->orderBy('id', 'desc')->get();
        $data['reviews_from_hosts'] = Reviews::with('users', 'properties')->where(['receiver_id'=>$user_id, 'reviewer'=>'host'])->orderBy('id', 'desc')->get();

        $data['reviews_count'] = $data['reviews_from_guests']->count() + $data['reviews_from_hosts']->count();

        $data['details'] = UserDetails::where('user_id', $user_id)->pluck('value', 'field')->toArray();
        $newuser = $this->svuser->userinfo();
		if($newuser == "success")
    	{
    	    $newuser1 = $this->svuser->user_bookings();
    	}
    ...
    public function transactionHistory(TransactionDataTable $dataTable)
    {
		...
		$data['hostpenalty'] = Penalty::where(['user_id'=>Auth::user()->id])->get();
    ...
    public function facebookConnect(Request $request)
    {
        ...
        UsersVerification::where('user_id', Auth::user()->id)
                           ->update([
                               'facebook' => 'yes',
                               'fb_id'    => $facebook_id,
                            ]);
    ...
    public function reviewsByYou(Request $request)
    {
        ...
        })->has('reviews', '<', 1)->paginate(Session::get('row_per_page'), ['*'], 'write');
    ...
    {
        $data          		= [];
        $data['title'] 		= 'Verify your document';
		$data['document']	= DB::table('sv_doc_verification')
								->where('user_id', Auth::user()->id)
								->get();
        return view('users.doc_verification', $data);
    }
	...
	public function doc_upload(Request $request)
	{
        ...
        $profileImage = $name;
        // Define upload path
        $destinationPath = public_path('images/doc/'); // upload path
	...
	public function doc_destroy(Request $request)
	{
		...
		if($edit_id !=0)
		{
			$gallery_image =DB::table('sv_doc_verification')
					->where('id',$edit_id)
					->delete();
			$path=public_path('images/doc/'.$filename);
	    ...
	    DB::delete('delete from sv_doc_verification where doc = ?',[$filename]);

        unlink($path);
        return response()->json(['success'=>'1']);;
	}

	public function docremove(Request $request)
	{
        $id = $request->did;
		DB::table('sv_doc_verification')->where('id',$id)->where('user_id',Auth::user()->id)->delete();
    }

    public function user_delete(Request $request)
    {
	    User::where(['id' => $request->id])->update(['deleted_status' => 'Yes' ]);
