...
class BookingController extends Controller
{
    ...
    */
    public function index(Request $request)
    {
        ...
        $data['booking_info']    = BookingDetails::where('booking_id', $data['booking_id'])->where('field', 'price')->first();

        $data['booking_packages'] = DB::table('booking_packages')->where('booking_id', $data['booking_id'])->get();

        if (! $data['result'] || $data['result']->host_id != Auth::user()->id ) {
            abort('404');
        }
        $data['price_list']         = json_decode($this->helper->get_price($data['result']->property_id, $data['result']->start_date, $data['result']->end_date, $data['result']->guest, true));
    ...
    */
    public function requestPayment(Request $request){
        ...
        $data['family_price']     = $booking->time_slot;
        ...
        $data['price_list']       = json_decode($this->helper->get_price($data['property_id'], $data['checkin'], $data['checkout'], $data['number_of_guests'],  $booking->time_slot));
        ...
        $data['booking_packages'] = DB::table('booking_packages')->where('booking_id', $request->id)->get();

        $msg = $this->helper1->booking();
		if($msg == "success")
    	{
    	    $msg1 = $this->helper1->active_booking();
    	}
    ...
    */
    public function requested(Request $request)
    {
        ...
        $data['date_price']       = json_decode($data['booking_details']->date_with_price);

        $bookid = $data['booking_details']->id;
        $data['booking_info']    = BookingDetails::where('booking_id', $bookid)->where('field', 'price')->first();

        $data['booking_packages'] = DB::table('booking_packages')->where('booking_id', $bookid)->get();
    ...
    */
    ...
    public function expire(Request $request)
    {
        ...
        $booking = Bookings::find($request->id);
  		$cancel_count = Bookings::where('host_id', Auth::user()->id)->where('cancelled_by', 'Host')->where('cancelled_at', '>=', DB::raw('DATE_SUB(NOW(), INTERVAL 6 MONTH)'))->get()->count();
    ...
    */
    ...
    public function hostCancel(Request $request, EmailController $email)
    {
        ...
        if (isset($payount->id)) {
            ...
            if ($bookings->booking_type != 'instant' ) {
                $start_date = new DateTime($bookings->start_date);
                //$panalty_date = new DateTime(date('Y-m-d H:i:s', strtotime('-7 days')));

                $curr_date    = date("Y-m-d");
                $start_date1  = date('Y-m-d', strtotime($bookings->start_date));
                $panalty_date = date('Y-m-d', strtotime('-7 days', strtotime($start_date1)));
                ...
                if ($curr_date >= $panalty_date && $curr_date <= $start_date1) {
                  //less then 7 days
