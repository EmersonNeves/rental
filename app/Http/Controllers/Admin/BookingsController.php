...
class BookingsController extends Controller
{
    ...
    public function index(BookingsDataTable $dataTable, Request $request)
    {
        ...
        $different_currency_total_initial  = $total_bookings_currency->select('bookings.currency_code as currency_code', DB::raw('SUM(bookings.total) AS total_amount'))->where('bookings.status','Accepted')->groupBy('currency_code');
        ...
        isset(request()->payment) ? $data['allpayments'] = request()->payment : $data['allpayments'] = ''; //CUSTOM
        return $dataTable->render('admin.bookings.view', $data);
    }
    ...
    public function details(Request $request)
    {
		...
		$data['fees'] = PropertyFees::pluck('value', 'field')->toArray();
    ...
    public function pay(Request $request)
    {
        ...
        $host_deduction_amt = $request->host_deduction_amt;
        ...
        if ($request->user_type == 'guest') {
            ...
            $amount                    = $booking->$host_deduction_amt;
        ...
        //$balance = ( $walletBalance->balance - $request->refund_amt );
        //$balance = ( $walletBalance->balance - $booking->original_host_payout );

        $balance = ( $walletBalance->balance - $host_deduction_amt );
        Wallet::where(['user_id' => $booking->host_id])->update(['balance' => $balance]);

		$walletBalance_user = Wallet::where('user_id',$booking->user_id)->first();
        $balance1 = ( $walletBalance_user->balance + $request->refund_amt );
        Wallet::where(['user_id' => $booking->user_id])->update(['balance' => $balance1]);


        $email = new EmailController;
        $email->payout_sent($booking_id);
        $this->helper->one_time_message('success', ucfirst($request->user_type).' Updated successfully');
        return redirect('admin/bookings/detail/'.$booking_id);
    }

	public function penalty_accept($bookid,$hostid)
	{
		Penalty::where(['booking_id'=>$bookid,'user_id'=>$hostid])->update(['status'=>'Completed']);
		$this->helper->one_time_message('success', 'Updated successfully');
        return redirect('admin/bookings/detail/'.$bookid);
	}

	public function penalty_decline($bookid,$hostid)
	{
		$walletBalance = Wallet::where('user_id',$hostid)->first();
		$penalty_amt   = Penalty::where('booking_id',$bookid)->where('user_id', $hostid)->where('status', 'Pending')->first();

        $balance 	   = ( $walletBalance->balance - $penalty_amt->amount );
