...
class TripsController extends Controller
{
    ...
    public function myTrips(Request $request)
    {
        ...
        $sv = $this->trips->active_trips();
		if($sv == "success")
    	{
    	    $sv1 = $this->trips->trips_count();
    	}
    ...
    {   
        ...
        if ($now <  $booking_start) {
            ...
            $this->helper->one_time_message('success', trans('messages.error.booking_cancel_error'));
            ...
            $email->bookingCancellationguest($bookings->id);
    ...
    public function receipt(Request $request)
    {
        ...
        $data['booking_details']  = BookingDetails::where('booking_id', $data['booking']->id)->get();
        $data['booking_details_price']  = BookingDetails::where('booking_id', $data['booking']->id)->where('field', 'price')->first();
        $data['booking_packages'] = DB::table('booking_packages')->where('booking_id', $data['booking']->id)->get();
