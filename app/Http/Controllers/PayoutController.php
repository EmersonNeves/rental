...
class PayoutController extends Controller
{
    ...
    public function setting(Request $request)
    {
      ...
      if (!$request->isMethod('post')) {
          ...
          //PIX
          elseif ($request->payout_type == 7) {

            $rules = array(
              'pix_type' => 'required|max:50',
              'pix_key'  => 'required|max:100',
            );
            ...
            if ($validator->fails()) {
              ...
              $payoutSetting->pix_type  = $request->pix_type;
              $payoutSetting->pix_key   = $request->pix_key;
              $payoutSetting->save();
              ...
              $paymentAccount->currency_code         = $currency_code;
              $paymentAccount->payment_method_id     = 7;
              $paymentAccount->selected              = 'No';
              $paymentAccount->save();
          ...
          //TED
          }elseif ($request->payout_type == 8) {

            $rules = array(
              'ted_name'      => 'required|max:50',
              'ted_document'  => 'required|max:14',
              'ted_bankname'  => 'required|max:50',
              'ted_bankcode'  => 'required|max:5',
              'ted_agency'    => 'required|max:10',
              'ted_account'   => 'required|max:20'
            );

            $fieldNames = array(
              'ted_name'      => 'Nome do Recebedor',
              'ted_document'  => 'CPF',
              'ted_bankname'  => 'Nome do Banco',
              'ted_bankcode'  => 'Código do Banco',
              'ted_agency'    => 'Agência',
              'ted_account'   => 'Conta Corrente'
            );
            ...
            if ($validator->fails()) {
              ...
              $payoutSetting->type                = $request->payout_type;
              $payoutSetting->email               = Auth::user()->email;
              $payoutSetting->ted_holder_name     = $request->ted_name;
              $payoutSetting->ted_holder_document = $request->ted_document;
              $payoutSetting->ted_bank_name       = $request->ted_bankname;
              $payoutSetting->ted_bank_code       = $request->ted_bankcode;
              $payoutSetting->ted_agency          = $request->ted_agency;
              $payoutSetting->ted_account         = $request->ted_account;
              $payoutSetting->save();
              ...
              $paymentAccount->account               = Auth::user()->email;
              $paymentAccount->currency_code         = $currency_code;
              $paymentAccount->payment_method_id     = 8;
              $paymentAccount->selected              = 'No';
              $paymentAccount->save();
