...
class PaymentController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        //CUSTOM
		    $data['getnet_status'] = Settings::where('name', 'getnet_status')
                                   ->where('type', 'Getnet')->first();
        ...
        $travel_credit            = 0;
        $data['current_lang']     = Session::get('language');
        ...
        $data['title']            = trans('messages.payment.custom_pay_for_your_reservation');
    ...
    public function createBooking(Request $request)
    {
        ...
        //CUSTOM: GETNET
        else if ($request->payment_method == 'getnet') {
          ...
          $getnet_metadata = Settings::where('type','Getnet')
                                     ->pluck('value','name');

          //GETNET
          $data['getnet_checkout_url']             = $getnet_metadata['getnet_checkout_url'];
          $data['getnet_seller_id']                = $getnet_metadata['getnet_seller_id'];
          $data['getnet_token']                    = $this->AuthGetnetPayment();
          $data['getnet_payment_methods_disable']  = json_encode(array_map(fn($v) => trim($v),explode(',',$getnet_metadata['getnet_disabled_methods'])));
          $data['getnet_amount']                   = number_format((float)$data['price_list']->total,2);
          $data['getnet_customerid']               = Auth::user()->id;
          $data['getnet_orderid']                  = Auth::user()->id.'-'.$data['property_id'].'-'.$from->format('Ymd').'-'.$to->format('Ymd');
          $data['getnet_installments']             = $getnet_metadata['getnet_installments'];
          $data['getnet_customer_first_name']      = substr(Auth::user()->first_name,0,40);
          $data['getnet_customer_last_name']       = substr(Auth::user()->last_name,0,80);
          $data['getnet_customer_email']           = Auth::user()->email;
          $data['getnet_customer_phone_number']    = substr(Auth::user()->phone,0,15);
          $data['getnet_customer_document_type']   = 'CPF';
          $data['getnet_customer_document_number']       = $request->payment_document_number;
          $data['getnet_customer_address_street']        = $request->payment_address_street;
          $data['getnet_customer_address_number']        = $request->payment_address_number;
          $data['getnet_customer_address_complement']    = $request->payment_address_complement;
          $data['getnet_customer_address_neighborhood']  = $request->payment_address_neighborhood;
          $data['getnet_customer_address_city']          = $request->payment_address_city;
          $data['getnet_customer_address_zipcode']       = $request->payment_address_zipcode;
          $data['getnet_customer_address_country']       = $request->payment_address_country;
          $data['getnet_customer_address_state']         = $request->payment_address_country == 'BR' ? $request->payment_address_state_br : $request->payment_address_state;
          $data['getnet_card_caixa']               = $getnet_metadata['getnet_card_caixa'] == '1' ? 'true' : 'false';
          $data['getnet_items']                    = json_encode([(object)[
            'name' => $data['result']->name,
            'description' =>
              $data['result']->name.' | '. //Property
              $data['nights'].' '.trans('messages.payment.night').' | '. //Nights
              $data['checkin'].' - '.$data['checkout'].' | '. //Checkin/Checkout
              $data['number_of_guests'], //Guests
            'value' => $data['getnet_amount'],
            'quantity' => $data['nights'],
            'sku' => $data['property_id']
          ]]);

          //BOOKING
          if ($data['getnet_token'])
          {
            $booking = Bookings::where('property_id',$request->property_id)
            ...
            if (!$booking)
              $code = $this->store([
                ...
                'first_name'       => $request->first_name,
                'last_name'        => $request->last_name,
                'postal_code'      => $request->payment_address_zipcode,
                'country'          => $request->payment_address_country,
                'message_to_host'  => $message_to_host,
                'payment_method_id'      => PaymentMethods::where('name',$request->payment_method)->first()->id,
                'payment_document_number'      => $request->payment_document_number,
                'payment_address_street'       => $request->payment_address_street,
                'payment_address_number'       => $request->payment_address_number,
                'payment_address_complement'   => $request->payment_address_complement,
                'payment_address_neighborhood' => $request->payment_address_neighborhood,
                'payment_address_city'         => $request->payment_address_city,
                'payment_address_zipcode'      => $request->payment_address_zipcode,
                'payment_address_country'      => $request->payment_address_country,
                'payment_address_state'        => $request->payment_address_country == 'BR' ? $request->payment_address_state_br :  $request->payment_address_state
              ]);
            else
            {
              $booking->payment_method_id = PaymentMethods::where('name',$request->payment_method)->first()->id;
              $booking->payment_document_number      = $request->payment_document_number;
              $booking->payment_address_street       = $request->payment_address_street;
              $booking->payment_address_number       = $request->payment_address_number;
              $booking->payment_address_neighborhood = $request->payment_address_neighborhood;
              $booking->payment_address_complement   = $request->payment_address_complement;
              $booking->payment_address_city         = $request->payment_address_city;
              $booking->payment_address_zipcode      = $request->payment_address_zipcode;
              $booking->payment_address_country      = $request->payment_address_country;
              $booking->payment_address_state        = $request->payment_address_country == 'BR' ? $request->payment_address_state_br :  $request->payment_address_state;
              $booking->save();

              BookingDetails::where(['id' => $booking->id])->update(['value' => $request->payment_address_country]);
            }

            $data['getnet_orderid'] = $code ?? $booking->code;
          }

          return view('payment.getnet',$data);
        }
...
exit; */
        ...
        //CUSTOM
        $booking->payment_document_number      = $data['payment_document_number']      ?? null;
        $booking->payment_address_street       = $data['payment_address_street']       ?? null;
        $booking->payment_address_number       = $data['payment_address_number']       ?? null;
        $booking->payment_address_neighborhood = $data['payment_address_neighborhood'] ?? null;
        $booking->payment_address_complement   = $data['payment_address_complement']   ?? null;
        $booking->payment_address_city         = $data['payment_address_city']         ?? null;
        $booking->payment_address_zipcode      = $data['payment_address_zipcode']      ?? null;
        $booking->payment_address_country      = $data['payment_address_country']      ?? null;
        $booking->payment_address_state        = $data['payment_address_state']        ?? null;
    ...
    public function update($data){
        ...
        $property_price_temp = PropertyPrice::where('property_id', $data['property_id'])->first();
        $days                = $this->helper->get_days(setDateForDb($data['checkin']), setDateForDb($data['checkout']));
        $code                = $data['code'] ?? $this->helper->randomCode(6);
        ...
        //CUSTOM
        $booking->payment_document_number      = $data['payment_document_number']      ?? $booking->payment_document_number ;
        $booking->payment_address_street       = $data['payment_address_street']       ?? $booking->payment_address_street;
        $booking->payment_address_number       = $data['payment_address_number']       ?? $booking->payment_address_number;
        $booking->payment_address_neighborhood = $data['payment_address_neighborhood'] ?? $booking->payment_address_neighborhood;
        $booking->payment_address_complement   = $data['payment_address_complement']   ?? $booking->payment_address_complement;
        $booking->payment_address_city         = $data['payment_address_city']         ?? $booking->payment_address_city;
        $booking->payment_address_zipcode      = $data['payment_address_zipcode']      ?? $booking->payment_address_zipcode;
        $booking->payment_address_country      = $data['payment_address_country']      ?? $booking->payment_address_country;
        $booking->payment_address_state        = $data['payment_address_state']        ?? $booking->payment_address_state;
    ...
    /**
     * CUSTOM: Authenticates on the Getnet service.
     *
     * @return string
     */
    public function AuthGetnetPayment()
    {
      $getnet_metadata = Settings::where('type','Getnet')
                                  ->pluck('value','name');

      $getnet_auth_url      = $getnet_metadata['getnet_auth_url'];
      $getnet_client_id     = $getnet_metadata['getnet_client_id'];
      $getnet_client_secret = $getnet_metadata['getnet_client_secret'];

      $response = Http::asForm()                                                                      //Content-Type: 'x-www-form-urlencoded'
                      ->withToken(base64_encode($getnet_client_id.':'.$getnet_client_secret),'Basic') //Authorization: 'Basic ...'
                      ->post($getnet_auth_url,[
                        'scope' => 'oob',
                        'grant_type' => 'client_credentials'
                      ])
                      ->json();

      return isset($response['access_token']) ? ($response['token_type'].' '.$response['access_token']) : '';
    }


    /**
     * CUSTOM: Handles the GETNET payment notification.
     *
     * @return void
     */
    public function HandleGetnetPayment(Request $request)
    {
      $booking = Bookings::where('code',$request->order_id ?? '0')
                         ->where('user_id',$request->customer_id ?? 0)
                         ->first();

      if (!$booking)
        return \Log::info('Reserva não encontrada: '.$request->order_id);
      ...
      Transactions::insert([
        ...
        'created_at' => $request->authorization_timestamp,
      ...
      // [2024-02-06 02:58:30] local.INFO: array (
      //   'payment_type' => 'credit',
      //   'customer_id' => '113',
      //   'order_id' => '9Kz4Ag',
      //   'payment_id' => '6b97ff80-e913-41b0-93f2-75970b5511b2',
      //   'amount' => '11700',
      //   'status' => 'APPROVED',
      //   'number_installments' => '1',
      //   'acquirer_transaction_id' => '000235514940',
      //   'authorization_timestamp' => '2024-02-06T02:58:22.000Z',
      //   'brand' => 'mastercard',
      //   'terminal_nsu' => '431100',
      //   'authorization_code' => '402015507582305650',
      // )
    }
}
