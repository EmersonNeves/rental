  ...
  <div class="row rpt border pt-3 mb-5 mt-2">
    <div class="col-md-3 col-sm-3 col-xs-12"><!-- card pt-4 mb-5 mt-2 rounded-3 -->
      ...
      <div class="col-md-3 col-sm-3 col-xs-12">
        ...
        <!-- CUSTOM -->
        <h5 class="margin-top20 mb-10">
          <?php
            $current_lang = Session::get('language');

            if ($current_lang == 'pt')
              echo abbrDatePT($booking->start_date);
            else
              echo $booking->startdate_dmy;
          ?>
        </h5>
    ...
    <!-- CUSTOM -->
    <?php
      $transactions = \App\Models\Transactions::where('booking_id',$booking->id)
    ...
    <div class="table-responsive mt-3">
      <table class="table table-bordered table-hover p-0 m-0">
        <thead class="thead-dark">
          <tr>
            <th colspan="6">{{ trans('messages.trips_receipt.custom_transactions') }}</th>
          </tr>
        </thead>
        <tbody class="border">
          @if($transactions->count() <= 0)
            <tr>
              <td
                colspan="6"
                class="text-center">
                Ainda não há nenhuma transação registrada para essa reserva.
              </td>
            </tr>
          @else
              ...
              <tr>
                <td
                  ...
                  <?php
                    ...
                    if ($current_lang == 'pt')
                      echo Carbon\Carbon::parse($transaction->created_at)
                                        ->locale('pt')
                                        ->format('d/m/Y h:i');
                    else
                ...
                <td
                  class="text-center">
                  {{ trans('messages.payment.custom_getnet_method_'.strtolower(json_decode($transaction->content)->payment_type)) }}
                </td>
                <td
                  class="text-center">
                  {{ $booking->currency->symbol.' '.number_format((int)(json_decode($transaction->content)->amount) / 100,2,',','.') }}
                </td>
                <td
                  class="text-center">
                  {{ trans('messages.payment.custom_getnet_status_'.strtolower($transaction->status)) }}
                </td>
              </tr>
              @if (isset(json_decode($transaction->content)->description_detail))
                <tr>
                  <td
                    colspan="5">
                    {{ json_decode($transaction->content)->description_detail }}
                  </td>
                </tr>
              @endif
            @endforeach
