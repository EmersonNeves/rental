...
<?php
        if(isset($_REQUEST['type']))
        {
            ...
            if ($start_date != '')
            {
              $customStartDate = Carbon\Carbon::parse(strtotime($start_date));
              $customStartDate = $lang == 'pt' ? $customStartDate->locale('pt') : $customStartDate;

              $customEndDate = Carbon\Carbon::parse(strtotime($end_date));
              $customEnddate = $lang == 'pt' ? $customEndDate->locale('pt') : $customEndDate;

              $customIniDay   = $customStartDate->day;
              $customIniMonth = $customStartDate->shortMonthName;
              $customEndDay   = $customEndDate->day;
              $customEndMonth = $customEndDate->shortMonthName;

              if ($start_date == $end_date)
                $any_week = "$customIniDay $customIniMonth";
              else
                $any_week = "$customIniDay $customIniMonth - $customEndDay $customEndMonth";
            }
            else
              $any_week = '';
        }
        ...
        //CUSTOM GUESTS + PETS
        $adults   = isset($_REQUEST['adults']) ? (int) $_REQUEST['adults']        : 0;
        $children = $adults > 0                ? (int) $_REQUEST['children'] ?? 0 : 0;
        $babies   = $adults > 0                ? (int) $_REQUEST['babies']   ?? 0 : 0;
        $pets     = $adults > 0                ? (int) $_REQUEST['pets']     ?? 0 : 0;
        $guests   = $adults + $children + $babies;

        if ($guests > 0)
        {
          $guests  = $guests." ".($guests > 1 ? trans('messages.home.custom_guests') : trans('messages.home.custom_guest'));
          $guests .= $pets > 0 ? (" + ".$pets." ".($pets > 1 ? trans('messages.home.custom_pets') : trans('messages.home.custom_pet'))) : "";
        }
        else
        {
          $guests = null;
        ...
        //echo $dlang = Session::get('home_type');
     ?>


<header class="header_area  animated fadeIn {{ $homepage_type }} <?php if($currentPaths=="index" or $currentPaths=="/" or $currentPaths =="old-home"){?>homenav<?php } else {?>migrateshop_othernav<?php } ?>">
    <div class="main_menu">
               ...
               <a class="navbar-brand logo_h light_logo" aria-label="logo" href="{{ url('/') }}"><img src="{{ $light_logo ?? '' }}" alt="logo" class="img-130x32"></a>
		        ...
		        <form id="front-search-form1" method="post" action="{{url('search')}}" class="mob-search mt-3 mb-3 p-2">
					...
					<div class="row">
    					...
    					<input class="form-control p-3 text-14 m-0 font-weight-bold" id="any_week" placeholder="{{trans('messages.experience.any_week')}}" name="any_week" type="text" value="{{ $any_week }}">

						<input autocomplete="off" class="form-control p-3 text-14 m-0" id="any_guest" placeholder="{{trans('messages.experience.add_guest')}}" name="any_guest" type="text" value="{{ $guests }}">
...
<?php  //if($currentPaths != "properties/{slug}" && $currentPaths != "experiences/{slug}" && $currentPaths !='users/payout-list' && $currentPaths!='users/transaction-history' ) { ?>
<?php if($homepage_type != 'old_home' || $currentPaths!="/" && $currentPaths != "old-home" ) { ?>
                                        ...
                                        <label>{{trans('messages.home.guest')}}</label>
																				...
																				<div
																					...
																					<input
																						...
																						id="guests_counter"
																						type="text"
																						class="form-control p-3 text-14"
																						autocomplete="off"
																						placeholder="{{trans('messages.home.custom_add_guest')}}"
                                            ...
                                            value="{{ $guests }}"/>
																					...
																					<div
																						id="custom_guests_dropdown"
																						class="dropdown-menu py-4 px-5"
																						onclick="event.stopPropagation()"
																						aria-labelledby="dropdownMenuButton">

																						<!-- ADULTS -->
																						<div
																							class="my-4 d-flex justify-content-between filter-size">
																							<div
																								class="flex-grow-1">
																								<h4><b>{{ trans('messages.search.custom_adults') }}</b></h4>
																								<h5>{{ trans('messages.search.custom_adults_ages') }}</h5>
																							</div>
																							<div
																								class="flex-grow-0 d-flex">
																								<div
																									class="value-button filter-add-value"
																									onclick="subGuest('adults_counter')">
																									-
																								</div>
																								<input
																									disabled
																								...
																								<div
																									class="value-button filter-add-value"
																									onclick="addGuest('adults_counter')">
																									+
																								</div>
																							</div>
																						</div>
																						...
																						<div
																							...
																							<div
																								class="flex-grow-1">
																								<h4><b>{{ trans('messages.search.custom_children') }}</b></h4>
																								<h5>{{ trans('messages.search.custom_children_ages') }}</h5>
																							</div>
																							<div
																								class="flex-grow-0 d-flex">
																								<div
																									class="value-button filter-add-value"
																									onclick="subGuest('children_counter')">
																									-
																								</div>
																								<input
																									disabled
																								...
																								<div
																									class="value-button filter-add-value"
																									onclick="addGuest('children_counter')">
																									+
																								</div>
																							</div>
																						</div>
																						...
																						<div
																							...
																							<div
																								class="flex-grow-1">
																								<h4><b>{{ trans('messages.search.custom_babies') }}</b></h4>
																								<h5>{{ trans('messages.search.custom_babies_ages') }}</h5>
																							</div>
																							<div
																								class="flex-grow-0 d-flex">
																								<div
																									class="value-button filter-add-value"
																									onclick="subGuest('babies_counter')">
																									-
																								</div>
																								<input
																									disabled
																								...
																								<div
																									class="value-button filter-add-value"
																									onclick="addGuest('babies_counter')">
																									+
																								</div>
																							</div>
																						</div>
																						...
																						<div
																							...
																							<div
																								...
																								<h4><b>{{ trans('messages.search.custom_pets') }}</b></h4>
																							</div>
																							<div
																								class="flex-grow-0 d-flex">
																								<div
																									class="value-button filter-add-value"
																									onclick="subGuest('pets_counter')">
																									-
																								</div>
																								<input
																									disabled
            ...
            <div class="modal-body p-0">
                <ul class="mobile-side">
                    @if(Auth::check())
                        ...
                        <li><a href="{{ url('experience/create') }}">{{trans('messages.experience.add_experience')}}</a></li>
...
<script type="text/javascript">
 $('#location, .any_guest, #front-search-form1').click(function(e)
    ...
    $(document).on('click','.overlay',function()
    {
		...
		//CUSTOM - ADD GUEST
		function addGuest(target)
		{
			var target = $(`#${target}`);
			var value  = $(target).val() ?? 0;

			$(target).val(parseInt(value) + 1).trigger('input');
		}

		//CUSTOM - SUBTRACT GUEST
		function subGuest(target)
		{
			var target = $(`#${target}`);
			var value  = $(target).val() ?? 0;
		...
		//CUSTOM - GUESTS CHANGE
		$('#custom_guests_dropdown input[type="text"]').off('input').on('input', function({ target })
    {
			var adults   = parseInt($('#adults_counter').val())   ?? 0;
			var children = parseInt($('#children_counter').val()) ?? 0;
			var babies   = parseInt($('#babies_counter').val())   ?? 0;
			var pets     = parseInt($('#pets_counter').val())     ?? 0;

			if (adults <= 0)
      {
				if (target.id != 'adults_counter')
					adults = 1;
				else
        {
					children = 0;
      ...
      var guests = adults + children + babies;

			$('#adults_counter').val(adults);
			$('#children_counter').val(children);
			$('#babies_counter').val(babies);
			$('#pets_counter').val(pets);

      if (guests <= 0)
        return $('#guests_counter').val(null);

			$('#guests_counter').val((guests == 1 ? '1 Pessoa' : `${guests} Pessoas`) +
                               (pets   == 1 ? ' + 1 Pet' : (pets > 1 ? ` + ${pets} Pets` : '')));
		});
