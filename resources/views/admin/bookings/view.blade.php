								...
								<div class="box-body">
									<form class="form-horizontal" enctype='multipart/form-data' action="{{ url('admin/bookings') }}" method="GET" accept-charset="UTF-8">
										...
										<div class="col-md-12  d-none">
											<input class="form-control" type="text" id="startDate"  name="from" value="<?php if(isset($from)) { echo $from; } else { echo onlyFormat(date('d-m-Y')); } ?>" hidden>
											<input class="form-control" type="text" id="endDate"  name="to" value="<?php if(isset($to)) { echo $to; } else { echo onlyFormat(date('d-m-Y')); } ?>" hidden>
										</div>
										<div class="col-md-12">
											<div class="row">
												<div class="col-md-12 col-sm-12 col-xs-12">
													...
													<div class="col-md-2 col-sm-2 col-xs-12">
														...
														<select class="form-control" name="payment" id="payment">
															<option value="" >All</option>
															<option value="Approved" {{ $allpayments == "Approved" ? ' selected="selected"' : '' }}>Approved</option>
															<option value="Authorized"  {{ $allpayments == "Authorized"  ? ' selected="selected"' : '' }}>Authorized</option>
															<option value="Pending" {{ $allpayments == "Pending" ? ' selected="selected"' : '' }}>Pending</option>
															<option value="Confirmed"  {{ $allpayments == "Confirmed"  ? ' selected="selected"' : '' }}>Confirmed</option>
															<option value="Cancelled"  {{ $allpayments == "Cancelled"  ? ' selected="selected"' : '' }}>Cancelled</option>
                              <option value="Denied"  {{ $allpayments == "Denied"  ? ' selected="selected"' : '' }}>Denied</option>
                              <option value="Error"  {{ $allpayments == "Error"  ? ' selected="selected"' : '' }}>Error</option>
														</select>
													</div>
													...
													<div class="col-md-1 col-sm-2 col-xs-4">
														<br>
														<button type="submit" name="reset_btn" class="btn btn-default">Reset</button>
													</div>
												</div>
											</div>
										</div>
	...
	$(function() {
		...
		$(document).ready(function(){
			$('#dataTableBuilder_length').after('<div id="exportArea" class="col-md-4 col-sm-4 "><div class="row mt-m-2"><div class="btn-group"><a href="" title="CSV" id="csv">CSV</a><a href="" title="PDF" id="pdf">PDF</a></div><div class="btn btn-group btn-refresh"><a href="" id="tablereload" class="form-control"><span>Reset</span></a></div></div></div>');
		});
