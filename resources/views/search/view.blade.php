                                        ...
                                        <div>
                                            <div class="input-group input-group-lg mt-1">
                                                <div class="input-group-prepend">
                                                    <span class="input-group-text"><?php echo $default_currency->symbol == "" ? "$" : $default_currency->symbol ?></span>
                                                </div>
                                                <input
                                                    id="minPrice"
                                                    type="number"
                                                    ...
                                                    max="{{ $max_price }}"
                                                    step="1"
                                                    class="form-control mr-3 text-16"
                                                    style="font-size: 16px; min-width: 100px;"
                                ...
                                <div class="col-sm-12">
                                    <div class="row">
                                        <div class="col-sm-12">
                                            ...
                                            @foreach($amenities_categories as $category)
                                                <div class="row mt-4">
                                                    <div class="col-12">
                                                        <b>{{ $category->name }}</b>
                                                    </div>
                                                ...
                                                <div class="row mb-2">
                                                    @foreach($amenities->where('type_id',$category->id) as $row_amenities)
                                                        <div class="col-md-6 mt-2 rtl_text_right">
...
jQuery(document).ready(function() {
    ...
    <script type="text/javascript">
        ...
        {
            var range     = $('#price-range').attr('data-value').split(',');
            ...
            $('#minPrice').attr('placeholder',min_price);
            $('#maxPrice').attr('placeholder',max_price);
        });

        $("#minPrice").on("keyup",function(evt)
        {
            var max_price = $('#price-range').attr('data-value').split(',')[1];
            var min_price = $(evt.currentTarget).val();

            if (!min_price)
                min_price = $('#price-range').attr('data-slider-min');

            $('#price-range').slider('setValue',[parseInt(min_price),parseInt(max_price)]);
        });

        $("#maxPrice").on("keyup",function(evt)
        {
            var max_price = $(evt.currentTarget).val();
            var min_price = $('#price-range').attr('data-value').split(',')[0];

            if (!max_price)
                max_price = $('#price-range').attr('data-slider-max');
        ...
        function getProperties(map,url){
            ...
            var adults   = @json($adults   ?? null);
            var children = @json($children ?? 0);
            var babies   = @json($babies   ?? 0);
            var pets     = @json($pets     ?? 0);
            var guest    = adults && adults > 0 ? (parseInt(adults) + parseInt(children) + parseInt(babies)) : null;
