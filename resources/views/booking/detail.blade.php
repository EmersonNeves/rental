...
<div class="container-fluid container-fluid-90 margin-top-85 min-height">
	<div class="row mb-4">
		<div class="col-md-8 card p-5">
			...
			<?php
				$property_id  	  = $result->properties->id;
				$exp_booking_type = $result->properties->exp_booking_type;
				$booking_type     = $result->properties->type;
			?>
			<div class="row flex-column-reverse flex-md-row mt-4 m-0">
				<div class="col-md-9 p-0">
					<p class="text-justify">{{ $result->users->first_name }} {{ $result->users->last_name }} {{ trans('messages.booking_detail.accept_decline_request') }}</p>
          ...
          <!-- CUSTOM -->
          <p class="text-14 text-muted text-center">
            {{ trans('messages.booking_detail.member_since') }}
            <?php
              ...
              if ($current_lang == 'pt')
                ...
                echo $result->users->account_since;
            ?>
          </p>
				</div>
			</div>
		...
		<div class="col-md-4">
            ...
            <div class="text-14">
              <?php
                ...
                if ($current_lang == 'pt')
                {
                  echo '<b>'.abbrDatePT($result->start_date).'</b> até ';
                  echo '<b>'.abbrDatePT($result->end_date).'</b>';
                }
                else
                {
                  echo '<b>'.date('D, M d, Y',strtotime($result->start_date)).'</b> to ';
                  echo '<b>'.date('D, M d, Y', strtotime($result->end_date)).'</b>';
                }
              ?>
            </div>
					...
					<div class="border p-4 mt-3">
                        ...
                        @if($result->properties->type=="experience" && $result->properties->exp_booking_type=="1")
                         <div class="d-flex justify-content-between text-16">
    								<div>
    									<p class="pl-4">{{trans('messages.header.check_in')}}</p>
    								</div>
    								<div>
    									<p class="pr-4">{{ date('D, M d, Y', strtotime($result->start_date)) }} </p>
    								</div>
							    </div>
    						<div class="d-flex justify-content-between text-16">
    							<div>
						...
						@if($result->properties->type=="experience" && $result->properties->exp_booking_type=="2")
						...
						<div class="d-flex justify-content-between text-16">
							...
							<div>
								<p class="pr-4">{{ $result->time_slot}}</p>
							</div>
						</div>
                        @endif

                        @if($result->properties->type=="experience" && $result->properties->exp_booking_type=="3")
								...
								@if(isset($booking_packages))
									...
									<div>
									    ...
									    @if(isset($booking_info->value))
									    	<p class="pr-4">{!! $result->currency->symbol !!} {!! currency_fix($booking_info->value, $result->currency->code) !!}</p>
										@endif
						...
						@if($result->host_fee)
							<div class="d-flex justify-content-between text-16">
								<div>
									<p class="pl-4">{{ trans('messages.booking_detail.host_fee') }}
				...
				<div class="modal-body p-4">
					<div class="row">
						...
						<div class="col-md-12 mt-5 text-right">
							...
							<input type="hidden" name="id" id="booking_id" value="{{ $booking_id }}">
